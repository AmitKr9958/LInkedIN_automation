    ...
    def __init__(self,path="data/activity.sqlite3"):
        ...
        with sqlite3.connect(path) as db:
            db.execute("""CREATE TABLE IF NOT EXISTS applications(
              ...
              updated_at TEXT NOT NULL,notes TEXT DEFAULT '')"""); db.commit()
    ...
    def add(self,job_url,title="",company="",status="new"):
        ...
        with sqlite3.connect(self.path) as db:
            db.execute("""INSERT OR IGNORE INTO applications
              ...
              (job_url,title,company,status,datetime.now(timezone.utc).isoformat())); db.commit()

    def transition(self,job_url,new_status,notes=""):
        if new_status not in STATUSES: raise ValueError("invalid status")
        with sqlite3.connect(self.path) as db:
            row=db.execute("SELECT status FROM applications WHERE job_url=?",(job_url,)).fetchone()
            if not row: raise KeyError(job_url)
            if new_status not in TRANSITIONS[row[0]]: raise ValueError(f"invalid transition {row[0]} -> {new_status}")
            db.execute("UPDATE applications SET status=?,notes=?,updated_at=? WHERE job_url=?",
                       (new_status,notes,datetime.now(timezone.utc).isoformat(),job_url)); db.commit()

    def list(self,status=None):
        with sqlite3.connect(self.path) as db:
            if status: return db.execute("SELECT * FROM applications WHERE status=? ORDER BY updated_at DESC",(status,)).fetchall()
